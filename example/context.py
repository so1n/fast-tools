...
class ContextModel(ContextBaseModel):
    request_id: str = HeaderHelper("X-Request-Id", default_func=lambda request: str(uuid.uuid4()))
    ip: str = HeaderHelper("X-Real-IP", default_func=lambda request: request.client.host)
