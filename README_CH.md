...
class ContextModel(ContextBaseModel):
    ...
    request_id: str = HeaderQuery(
        'X-Request-Id',
        default_func=lambda request: str(uuid.uuid4())  # 如果获取不到数据则会使用default_func的返回值
    )
    ip: str = HeaderQuery(
        'X-Real-IP',
...
@app.get("/api/users/login")
@cache(redis_helper, 60, after_cache_response_list=[cache_control])  # 在返回缓存响应时,在http头添加缓存时间
